class PetHistoriesController < ApplicationController
  ...
  def set_pet_options
    # mapeo para el select de mascotas que muestro en el form anidado de historial.
    @pet_options = Pet.all.map{ |u| [ u.name, u.id ] }
  end 
  ...
  def create
    ...
    @pet_history = @pet.pet_history.create(pet_history_params)

    respond_to do |format|
      if @pet_history.save
        format.html { redirect_to pet_path(@pet), notice: 'Pet history was successfully created.' }
  ...
  def update
    #p "UPDATE"
    respond_to do |format|
        #p "@pet_history.update(pet_history_params) = #{pet_history_params}"
        if @pet_history.update(pet_history_params)
        #format.html { redirect_to @pet_history, notice: 'Pet history was successfully updated.' }
        #p "Pet history was successfully updated."
        #p "client_pet_path = #{client_pet_path}"
        format.html { redirect_to client_pet_path, notice: 'Pet history was successfully updated.' }
